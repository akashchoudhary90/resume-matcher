...
- **Phase-5 C1 addendum — the under-networked ROSTER runs under this same governed program** (privacy F7). Phase 4's mitigation surfaced *coverage aggregates*; `GET /api/coordinator/under-networked` surfaces an individual-level **list of named students who are structurally disadvantaged**, which is a materially bigger disclosure and is governed accordingly:
  - **Cohort:** `graph_discoverable` ∩ **`network_analytics`** consent. A per-person listing is analytics *about that person's network position*, so it takes the analytics consent on top of discoverability — a student can be discoverable for intros without appearing on a roster of the under-connected.
  - **Trigger stays purely structural** (zero shareable edges). No self-ID attribute, and no `grad_year` (privacy F2 — the column does not exist and sits in `PROTECTED_KEYS`), is ever read near this query; test-enforced.
  - **Every read is access-logged** (`events` row `action='under_networked_viewed'`, actor = the coordinator). The roster of a disadvantaged class is not a free read: who looked, and when, is itself on the record.
  - **Monitoring + shut-off, unchanged from this slice's program terms:** the roster exists to be *acted on* (mentor offers, bridges) and its effect is measured by the before/after impact ratios and the conversion funnel. If the program does not close the access gap, it is shut off — the roster is not retained as a standing surveillance list of the poorly-connected.
...
- **Folded requirements (R1-HIGH retention; both-reviewer legal):**
  ...
  - **Graph erasure** (`NetworkStore.delete_my_network`, shipped in Slice Z): hard-delete the member's edges, vouches (as voucher and as subject), intro rows, discovery identity, broker blocks; write `graph_suppressions(user_id, 'member_deleted')`; `intro_events` retain only opaque IDs + status (no PII to scrub). **Scope correction (Phase 5 A3):** Phase 4 shipped this GRAPH-only cascade and this document previously described it as an "erasure job on account deletion", which it was not — it never touched the account itself (resumes, applications, match results, profile, consents, tokens, notifications) and there was no route or operator tool that deleted an account at all. The full two-plane account cascade landed in Phase 5 as **`stores/erasure.py::erase_account`** (audit.db first, then ONE platform.db transaction with the tombstone inside it), reachable from `DELETE /api/account` (password step-up) and `scripts/dsr_erase.py`; the contract is specified in [`PHASE5.md`](PHASE5.md) §6 and pinned by `tests/test_phase5_erasure.py`. `delete_my_network` remains the narrower member-initiated "delete my network" control.
  ...
  - **Out-of-scope gates carried forward from Phase 5** (documented, deliberately NOT built — PHASE5.md scope line): OIDC SSO, a real KMS for the tokenizer pepper, the Postgres port, Playwright coverage, vision-PDF, the multi-role picker, and URL ingest. Each is a decision to take before a real cohort, not an omission to discover later. Operationally: `RM_ENV=prod` (A16) makes an unset/weak admin password and an explicitly insecure cookie a **boot failure**, but `deploy/cohost/docker-compose.cohost.yml` does not forward `RM_ENV` yet — adding that one line is the recorded, deliberate act that turns the demo stack into a prod stack.
- **Tests:** `test_retention_purges_expired`. `test_erasure_cascade_leaves_no_pii` (graph scope). `test_intro_events_pii_free_after_erasure`. Phase 5 adds `tests/test_phase5_erasure.py` for the full account cascade (per-table no-residue sweep, logs anonymized not deleted, dry-run inert, business records survive behind the sentinel).
- **Exit:** retention is enforced, graph erasure is true and complete (**account erasure lands in Phase 5 A3** — see the scope correction above), and the documented legal gates are recorded as launch blockers.
